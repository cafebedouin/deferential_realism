...
**The critical distinction:** True Mountains have **zero or minimal extraction** (ε ≤ 0.15) and **zero or minimal suppression** (Supp ≤ 0.05). If a constraint requires active enforcement or extracts heavily from participants, it's not a natural law—it's a constructed constraint claiming inevitability.
...
```
■C[I] ↔ ε(C) ≤ 0.15 ∧ Supp(C) ≤ 0.05 ∧ Immutable(C, I.T, I.E)
```

**Components:**

- **ε(C) ≤ 0.15**: Base extractiveness at coordination floor
...
> **Shadow Mode Reminder:** This gate runs AFTER metric-based classification in `classify_from_metrics/6`. It does not modify the core classification logic—it operates in the signature override layer (`structural_signatures.pl`) and can demote a claimed Mountain to Constructed Constraint based on coupling topology.
>
...
**Use when:** Constraint genuinely unchangeable from your index (ε ≤ 0.15, Supp ≤ 0.05, immutable)
...
BUT ε = 0.60 fails rope_epsilon_ceiling (0.15)
...
```
⊞C[I] ↔ χ(C, I.P, I.S) ≤ 0.35 ∧ ε(C) ≤ 0.15 ∧ Changeable(C, I.T, I.E)
```
...
- **ε(C) ≤ 0.15**: Base extraction at coordination floor
  - Same as Mountain threshold—genuine coordination shouldn't extract
...
- **Changeable(C, I.T, I.E)**: Not immutable—can be modified or replaced
...
>
> - **Rope** (from metrics): χ ≤ 0.35, ε ≤ 0.15 — passes thresholds
...
**Use when:** Genuine coordination with minimal extraction (χ ≤ 0.35, ε ≤ 0.15, changeable)
...
>
> **Extraction is often hidden.** If a constraint fails the Boltzmann Factorization test (coupling score > 0.15), it couples independent dimensions in ways that natural laws cannot—this is mathematical evidence of construction, even if it mimics a natural law or claims symmetric benefits.
>
> **The Core Insight:**
>
...
**Distinction from Tangled Rope:** Snares extract heavily (ε ≥ 0.46); Tangled Ropes have genuine coordination (ε typically 0.40-0.60, requires Coord predicate)
...
- **ε(C) ≥ 0.50**: High base extraction
  - Distinguishes from Rope (ε ≤ 0.15)
...
Real-world coordination is messy. Pure coordination (Rope, ε ≤ 0.15) is rare. Pure extraction (Snare, ε ≥ 0.46, no coordination) is also rare. Most constraints fall in the middle: they coordinate something while extracting asymmetrically.
...
**Use when:** Genuine coordination exists but extraction high (0.40 ≤ χ ≤ 0.90, ε ≥ 0.50, both Coord and Asymmetric)
...
```
⊡C[I] ↔ χ(C, I.P, I.S) ≤ 0.30 ∧ Coord(C) ∧ Sunset(C) ∧ Theater(C) ≤ 0.70
```
...
- **χ ≤ 0.10**: Minimal effective extraction (inactive)
...
- **Rope**: χ ≤ 0.35, ε ≤ 0.15, Theater < 0.70 (real coordination)
- **Piton**: χ ≤ 0.10, ε > 0.10, Theater ≥ 0.70 (theater dominates)

##### Formal Definition

```
⊟C[I] ↔ χ(C, I.P, I.S) ≤ 0.10 ∧ ε(C) > 0.10 ∧ Theater(C) ≥ 0.70
```

**Components:**
- **χ ≤ 0.10**: Minimal effective extraction
...
- **Piton**: Low extraction (χ ≤ 0.10), mostly theater (≥ 0.70) → **inert decay** → energy better spent building alternatives
...
**Use when:** Degraded constraint with high theater (χ ≤ 0.10, ε > 0.10, Theater ≥ 0.70)
...
**[§II.B COMPLETE - Six Core Modal Operators]**
...
If a constraint:
- Passes Rope thresholds (χ ≤ 0.35, ε ≤ 0.15), BUT
...
The power modifier π(P) adjusts base extractiveness ε(C) based on structural power position.
...
CD(C, t_drift) ≡ 
    ...
    ∧ coupling_score(C, t_drift) - coupling_score(C, t₀) ≥ 0.10
...
**│ Mountain > Snare > Scaffold > Rope > Tangled > Piton   │**
**└─────────────────────────────────────────────────────────┘**

**Why this ordering?**

1. **Mountain first:** Most restrictive gate (ε ≤ 0.15, Supp ≤ 0.05, immutable). Must rule out before considering changeable types.
...
2. **Detection patterns only refine or demote** — Signatures in structural_signatures.pl can override but never modify core logic
...
**On Thresholds:** The values in logic_thresholds.md (ε ≤ 0.15 for Rope, χ ≥ 0.66 for Snare, etc.) are **calibration points in a noisy measurement regime**, not Platonic ideals. They represent empirically observed boundaries in the 691-constraint corpus but remain subject to:
...
- ✅ §II Basic Syntax (index structure, six types, detection patterns, π/σ modifiers)
