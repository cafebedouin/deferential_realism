...
[SCENARIO MANAGER] Loading: testsets/antifragility_u3_exp_r2.pl...
[SCENARIO MANAGER] Performing Global Repair...

[REPAIR] Auditing vectors for: antifragility_u3_exp_r2...
  [FIXED] Imputed 24 missing vectors using domain priors

>>> INITIATING DR-AUDIT SUITE: antifragility_u3_exp_r2

--- [START] Data Verification ---
[OK] Ontology Schema matches.
Checking Interval: antifragility_u3_exp_r2 (0-10)
--- [END] Data Verification Complete ---
[OK] Verification passed.

--- PER-INDEX VALIDATION ---
  [INDEX MISMATCH] antifragility_u3_exp_r2 from context(agent_power(powerless),time_horizon(biographical),exit_options(trapped),spatial_scope(national)): declared=snare, computed=tangled_rope
  [INDEX MISMATCH] antifragility_u3_exp_r2 from context(agent_power(moderate),time_horizon(biographical),exit_options(arbitrage),spatial_scope(global)): declared=rope, computed=tangled_rope
  [INDEX OK] antifragility_u3_exp_r2 from context(agent_power(institutional),time_horizon(generational),exit_options(constrained),spatial_scope(national)): declared=tangled_rope, computed=tangled_rope
  [INDEX MISMATCH] antifragility_u3_exp_r2 from context(agent_power(analytical),time_horizon(civilizational),exit_options(analytical),spatial_scope(universal)): declared=mountain, computed=tangled_rope
...
--- Terminal State Predictions ---
  antifragility_u3_exp_r2 -> tangled_rope (confidence: low)
...
--- SCOPE EFFECT ANALYSIS ---
  Formula: χ = ε × f(d) × σ(S)
  antifragility_u3_exp_r2 (ε=0.75):
...
=== Logical Fingerprint: antifragility_u3_exp_r2 ===
...
--- ENRICHED OMEGA CONTEXT ---
  ...
  Omega: omega_perspectival_antifragility_u3_exp_r2
    Severity Score:    0.57
    Gap Class:         consensus
...
[STRUCTURAL SIGNATURE ANALYSIS]
  antifragility_u3_exp_r2: false_natural_law (confidence: high)
    → FALSE NATURAL LAW signature for antifragility_u3_exp_r2: Claims naturality (indexed_mountain_classification) but fails Boltzmann independence test. Coupling score=0.750 with 0 coupled dimension pairs. Excess extraction=0.6. This constraint is "physics-washed" — it appears natural but its coupling topology reveals structural construction.

Aggregate Magnitude (Kappa) at Tn: 0.53

[MANDATROPHY GAP ANALYSIS]
  (Full perspectival detail in Levels 1-2 above)
  antifragility_u3_exp_r2 (snare vs tangled_rope):
...
[OMEGA RESOLUTION SCENARIO GENERATION]
  ...
  │  Constraint: unknown
  │  Gap: Distinguishing between necessary evolutionary culling and predatory risk transfer.
  │
  │  RESOLUTION STRATEGY:
  ...
  ┌─ [omega_perspectival_antifragility_u3_exp_r2] CONCEPTUAL CLARIFICATION
  │  Constraint: antifragility_u3_exp_r2
  │  Gap: Constraint antifragility_u3_exp_r2 appears as snare to individuals but tangled_rope to institutions...
  │
  │  RESOLUTION STRATEGY:
  │  1. Map stakeholder perspectives:
  │     - Document how different actors perceive antifragility_u3_exp_r2
