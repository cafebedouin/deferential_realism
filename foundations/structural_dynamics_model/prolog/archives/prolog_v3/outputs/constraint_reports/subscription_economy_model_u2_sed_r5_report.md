...
[SCENARIO MANAGER] Loading: testsets/subscription_economy_model_u2_sed_r5.pl...
[SCENARIO MANAGER] Performing Global Repair...

[REPAIR] Auditing vectors for: subscription_economy_model_u2_sed_r5...
  [FIXED] Imputed 32 missing vectors using domain priors

>>> INITIATING DR-AUDIT SUITE: subscription_economy_model_u2_sed_r5

--- [START] Data Verification ---
[OK] Ontology Schema matches.
Checking Interval: subscription_economy_model_u2_sed_r5 (2010-2024)
--- [END] Data Verification Complete ---
[OK] Verification passed.

--- PER-INDEX VALIDATION ---
  [INDEX OK] subscription_economy_model_u2_sed_r5 from context(agent_power(powerless),time_horizon(biographical),exit_options(trapped),spatial_scope(national)): declared=snare, computed=snare
  [INDEX OK] subscription_economy_model_u2_sed_r5 from context(agent_power(institutional),time_horizon(biographical),exit_options(arbitrage),spatial_scope(global)): declared=rope, computed=rope
  [INDEX OK] subscription_economy_model_u2_sed_r5 from context(agent_power(moderate),time_horizon(biographical),exit_options(mobile),spatial_scope(national)): declared=snare, computed=snare
  [INDEX MISMATCH] subscription_economy_model_u2_sed_r5 from context(agent_power(analytical),time_horizon(civilizational),exit_options(analytical),spatial_scope(global)): declared=tangled_rope, computed=snare
...
================================================================
  ...
  subscription_economy_model_u2_sed_r5:
    [critical] extraction_accumulation
        Evidence: evidence(extraction_delta,0,14,0.3,0.68)
...
--- Terminal State Predictions ---
  subscription_economy_model_u2_sed_r5 -> tangled_rope (confidence: low)
...
--- SCOPE EFFECT ANALYSIS ---
  Formula: χ = ε × f(d) × σ(S)
  subscription_economy_model_u2_sed_r5 (ε=0.68):
...
=== Logical Fingerprint: subscription_economy_model_u2_sed_r5 ===
...
--- ENRICHED OMEGA CONTEXT ---
  ...
  Omega: omega_extraction_blindness_subscription_economy_model_u2_sed_r5
...
--- CLASSIFICATION CONVERGENCE ---
    ...
    Confidence:       0.1891 (borderline)
    Rival Type:       snare (P=0.8109)
    Margin:           -0.6219
...
--- DIAGNOSTIC VERDICT ---
  ...
  Tensions (2):
    ...
    drift: critical_drift([drift(extraction_accumulation,evidence(extraction_delta,0,14,0.3,0.68),critical),drift(coupling_drift,evidence(coupling_score,1.0,threshold,0.25,extraction_trend,increasing),critical),drift(purity_drift,evidence(current_purity,0.3541666666666667,decline_signals,[extraction_rising,coupling_above_threshold(1.0),theater_rising,excess_above_floor(0.5800000000000001)]),warning),drift(network_drift,evidence(drifting_neighbors,[contagion(subscription_economy_model_u2_sed_r1,0.006250000000000003,[extraction_rising,coupling_above_threshold(1.0),theater_rising,excess_above_floor(0.5800000000000001)]),contagion(subscription_economy_model_u2_sed_r2,0.00325,[extraction_rising,coupling_above_threshold(1.0),theater_rising,excess_above_floor(0.44999999999999996)])],effective_purity,0.3494166666666667,intrinsic_purity,0.3541666666666667),critical)])
...
[MANDATROPHY GAP ANALYSIS]
  ...
  subscription_economy_model_u2_sed_r5 (snare vs rope):
...
[OMEGA RESOLUTION SCENARIO GENERATION]
  ...
  ┌─ [omega_extraction_blindness_subscription_economy_model_u2_sed_r5] CONCEPTUAL CLARIFICATION
  │  Constraint: subscription_economy_model_u2_sed_r5
  │  Gap: Constraint subscription_economy_model_u2_sed_r5 appears extractive (Snare) to individuals but functional (Rope) to institutions...
  │
  ...
  │     - Who benefits from subscription_economy_model_u2_sed_r5?
  │     - Can you change/exit this constraint?
  │     - What would happen if you tried?
  │  2. Interview institutional actors (N=10+):
  │     - What function does subscription_economy_model_u2_sed_r5 serve?
