...
[SCENARIO MANAGER] Loading: testsets/epistemic_p_u3_exp_r1.pl...
[SCENARIO MANAGER] Performing Global Repair...

[REPAIR] Auditing vectors for: epistemic_p_u3_exp_r1...
  [FIXED] Imputed 32 missing vectors using domain priors

>>> INITIATING DR-AUDIT SUITE: epistemic_p_u3_exp_r1

--- [START] Data Verification ---
[OK] Ontology Schema matches.
Checking Interval: epistemic_p_u3_exp_r1 (1950-2020)
--- [END] Data Verification Complete ---
[OK] Verification passed.

--- PER-INDEX VALIDATION ---
  [INDEX MISMATCH] epistemic_p_u3_exp_r1 from context(agent_power(powerless),time_horizon(biographical),exit_options(trapped),spatial_scope(national)): declared=snare, computed=tangled_rope
  [INDEX MISMATCH] epistemic_p_u3_exp_r1 from context(agent_power(powerful),time_horizon(biographical),exit_options(mobile),spatial_scope(global)): declared=rope, computed=tangled_rope
  [INDEX MISMATCH] epistemic_p_u3_exp_r1 from context(agent_power(institutional),time_horizon(generational),exit_options(constrained),spatial_scope(global)): declared=tangled_rope, computed=naturalized
  [INDEX MISMATCH] epistemic_p_u3_exp_r1 from context(agent_power(analytical),time_horizon(civilizational),exit_options(analytical),spatial_scope(global)): declared=tangled_rope, computed=snare
...
================================================================
  ...
  epistemic_p_u3_exp_r1:
    [critical] extraction_accumulation
        Evidence: evidence(extraction_delta,1950,2020,0.2,0.68)
...
--- Terminal State Predictions ---
  epistemic_p_u3_exp_r1 -> tangled_rope (confidence: low)
...
=== Logical Fingerprint: epistemic_p_u3_exp_r1 ===
...
--- ENRICHED OMEGA CONTEXT ---
  ...
  Omega: omega_perspectival_epistemic_p_u3_exp_r1
...
--- CLASSIFICATION CONVERGENCE ---
    ...
    Confidence:       0.1643 (borderline)
    Rival Type:       snare (P=0.8357)
    Margin:           -0.6713
...
--- MAXENT SHADOW CLASSIFICATION ---
  ...
  Margin:        -0.6713
  Entropy:       0.2494
  Distribution:  snare: 0.836, tangled_rope: 0.164
...
--- DIAGNOSTIC VERDICT ---
  ...
  Tensions (2):
    ...
    drift: critical_drift([drift(extraction_accumulation,evidence(extraction_delta,1950,2020,0.2,0.68),critical),drift(coupling_drift,evidence(coupling_score,1.0,threshold,0.25,extraction_trend,increasing),critical),drift(purity_drift,evidence(current_purity,0.3125,decline_signals,[extraction_rising,coupling_above_threshold(1.0),theater_rising,excess_above_floor(0.66)]),warning),drift(network_drift,evidence(drifting_neighbors,[contagion(publishing_embargo,0.0027124999999999966,[extraction_rising,coupling_above_threshold(1.0),theater_rising,excess_above_floor(0.32999999999999996)])],effective_purity,0.31114375,intrinsic_purity,0.3125),critical)])
...
[META-LOGICAL AUDIT: ONTOLOGICAL FRAUD DETECTION]
  ! ALERT [informational]: perspectival_incoherence detected for epistemic_p_u3_exp_r1

[STRUCTURAL SIGNATURE ANALYSIS]
  epistemic_p_u3_exp_r1: false_ci_rope (confidence: high)
    → FALSE CI_ROPE signature for epistemic_p_u3_exp_r1: Appears to be rope (indexed_rope_classification) but fails 4 Boltzmann structural test(s): [boltzmann_non_compliant(1.0,0.25),scope_variant([snare,tangled_rope]),excess_above_floor(0.66),nonsensical_coupling(0.5)]. Coupling score=1.0. This constraint is "coordination-washed" — it hides extraction behind low metrics, distributed enforcement, or behavioral defaults.
    → Institutional beneficiary: researchers_with_positive_results
...
[MANDATROPHY GAP ANALYSIS]
  ...
  epistemic_p_u3_exp_r1 (snare vs tangled_rope):
...
[OMEGA RESOLUTION SCENARIO GENERATION]
  ...
  │  Constraint: unknown
  │  Gap: Whether the p<0.05 threshold is a necessary, if costly, heuristic or a purely pathological convention.
  │
  │  RESOLUTION STRATEGY:
  ...
  ┌─ [omega_perspectival_epistemic_p_u3_exp_r1] CONCEPTUAL CLARIFICATION
  │  Constraint: epistemic_p_u3_exp_r1
  │  Gap: Constraint epistemic_p_u3_exp_r1 appears as snare to individuals but tangled_rope to institutions...
  │
  │  RESOLUTION STRATEGY:
  │  1. Map stakeholder perspectives:
  │     - Document how different actors perceive epistemic_p_u3_exp_r1
