...
[SCENARIO MANAGER] Loading: testsets/antifragility_u2_exp_r3.pl...
[SCENARIO MANAGER] Performing Global Repair...

[REPAIR] Auditing vectors for: antifragility_u2_exp_r3...
  [FIXED] Imputed 24 missing vectors using domain priors

>>> INITIATING DR-AUDIT SUITE: antifragility_u2_exp_r3

--- [START] Data Verification ---
[OK] Ontology Schema matches.
Checking Interval: antifragility_u2_exp_r3 (0-10)
--- [END] Data Verification Complete ---
[OK] Verification passed.

--- PER-INDEX VALIDATION ---
  [INDEX OK] antifragility_u2_exp_r3 from context(agent_power(powerless),time_horizon(biographical),exit_options(trapped),spatial_scope(national)): declared=snare, computed=snare
  [INDEX MISMATCH] antifragility_u2_exp_r3 from context(agent_power(moderate),time_horizon(biographical),exit_options(arbitrage),spatial_scope(global)): declared=rope, computed=snare
  [INDEX MISMATCH] antifragility_u2_exp_r3 from context(agent_power(institutional),time_horizon(immediate),exit_options(constrained),spatial_scope(national)): declared=tangled_rope, computed=naturalized
  [INDEX MISMATCH] antifragility_u2_exp_r3 from context(agent_power(analytical),time_horizon(civilizational),exit_options(analytical),spatial_scope(global)): declared=tangled_rope, computed=snare
...
--- Terminal State Predictions ---
  antifragility_u2_exp_r3 -> tangled_rope (confidence: low)
...
--- SCOPE EFFECT ANALYSIS ---
  Formula: χ = ε × f(d) × σ(S)
  antifragility_u2_exp_r3 (ε=0.75):
...
=== Logical Fingerprint: antifragility_u2_exp_r3 ===
...
--- ENRICHED OMEGA CONTEXT ---
  ...
  Omega: omega_perspectival_antifragility_u2_exp_r3
    Severity Score:    0.73
...
[META-LOGICAL AUDIT: ONTOLOGICAL FRAUD DETECTION]
  ! ALERT [informational]: perspectival_incoherence detected for antifragility_u2_exp_r3

[STRUCTURAL SIGNATURE ANALYSIS]
  antifragility_u2_exp_r3: false_ci_rope (confidence: medium)
    → FALSE CI_ROPE signature for antifragility_u2_exp_r3: Appears to be rope (indexed_rope_classification) but fails 2 Boltzmann structural test(s): [boltzmann_non_compliant(0.75,0.25),excess_above_floor(0.7)]. Coupling score=0.75. This constraint is "coordination-washed" — it hides extraction behind low metrics, distributed enforcement, or behavioral defaults.
...
[MANDATROPHY GAP ANALYSIS]
  ...
  antifragility_u2_exp_r3 (snare vs tangled_rope):
    ! MANDATROPHY GAP: delta_chi = 0.85 (high)

[OMEGA RESOLUTION SCENARIO GENERATION]
  Generated 2 resolution scenario(s):

  ┌─ [extraction_necessity] CONCEPTUAL CLARIFICATION
  │  Constraint: unknown
  │  Gap: Whether the extraction is a necessary evolutionary feature or a predatory economic one.
  │
  │  RESOLUTION STRATEGY:
  ...
  ┌─ [omega_perspectival_antifragility_u2_exp_r3] CONCEPTUAL CLARIFICATION
  │  Constraint: antifragility_u2_exp_r3
  │  Gap: Constraint antifragility_u2_exp_r3 appears as snare to individuals but tangled_rope to institutions...
  │
  │  RESOLUTION STRATEGY:
  │  1. Map stakeholder perspectives:
  │     - Document how different actors perceive antifragility_u2_exp_r3
  │     - Identify source of divergence
  │  2. Gather evidence:
  │     - Empirical metrics (suppression, extraction, resistance)
  │     - Historical behavior patterns
