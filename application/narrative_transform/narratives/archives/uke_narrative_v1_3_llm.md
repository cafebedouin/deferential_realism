# UKE_Narrative v1.3 (LLM-Optimized)
...
**FOR LLMs:** Execute stages 0-4 sequentially. Reference `logic_narrative.md` for classification system and narrative applications. Reference `logic_thresholds.md` for canonical threshold values. Rest of document is operational guidance.
...
**Narrative adjustment is valid:** If tightening ε by ±0.10 creates sharper indexical variance — where the powerless character crosses into Snare while the moderate stays in Tangled Rope — that's better routing. The topology sharpens. Nobody is harmed because there is no ground truth to betray in a fictional constraint.
...
5. **Classify constraint type per character:**
   ...
   - Powerless character: higher χ → routes toward Snare experience
   - Moderate character: mid χ → routes toward Tangled Rope experience
   - Powerful character: low/negative χ → routes toward Rope experience
...
7. **(Optional) Identify structural physics opportunities:**
   ...
   False Mountain: Does any constraint claim to be natural/unchangeable
     ...
     → Potential dramatic reveal: "natural order" exposed as constructed
   
   Purity drift: Is any constraint visibly functional but silently degrading?
     → Potential slow-burn arc: system looks healthy, rot underneath
   
   Network contamination: Is any healthy constraint surrounded by
     degraded neighbors?
     → Potential contagion plot: good system poisoned by context
...
<constraint_map>
  <constraint id="C1" name="[name]">
    ...
    <structural_physics_opportunities>
      [Optional: False Mountain, purity drift, or contamination potential]
    </structural_physics_opportunities>
  </constraint>
...
4. **Select terminal attractor:**
   ...
   <attractor>
     ...
     <entry_condition>
       χ(Tangled Rope) stabilizes at moderate level for most characters
     </entry_condition>
     <stable_state>
...
5. **(Optional) Formalize structural physics arcs:**
   ```xml
   <structural_physics>
     <false_mountain constraint_id="C2">
       <presented_as>Mountain (natural order, immutable)</presented_as>
       <evidence_of_construction>
         Burden varies by power position (not scope-invariant).
         Requires active enforcement (Supp > 0.05).
       </evidence_of_construction>
       <reveal_trigger>Analytical character notices enforcement costs</reveal_trigger>
       <reclassification>Tangled Rope (coordination exists but extraction hidden)</reclassification>
     </false_mountain>
     
     <purity_drift constraint_id="C1">
       <initial_state>Functional Tangled Rope, purity moderate</initial_state>
       <drift_direction>Theater increasing, coordination narrowing</drift_direction>
       <visible_crisis>Type flip to Snare when purity crosses threshold</visible_crisis>
     </purity_drift>
     
     <network_contamination constraint_id="C3">
       <focal_constraint>Healthy Rope</focal_constraint>
       <contaminating_neighbors>[List degraded constraints]</contaminating_neighbors>
       <propagation>Effective purity drops despite intrinsic health</propagation>
     </network_contamination>
   </structural_physics>
   ```
...
<formal_specification source="[story_name]">
  ...
  <structural_physics>
    [Optional: False Mountain, purity drift, contamination arcs]
  </structural_physics>
...
☐ Structural physics arcs formalized (if identified in Stage 0)
```
...
2. **Re-instantiate each constraint in new setting:**
   ...
   Must preserve:
   - Relative ε (routing should land in same type zone)
   ...
   ε tolerance: ±0.10 from source, BUT if adjusting ε within
   that range sharpens indexical variance, prefer the sharper value.
   The goal is routing to the right dramatic experience per character.
...
5. **(If Stage 1 includes structural physics arcs) Naturalize them:**
   
   ```
   False Mountain reveal:
     → Design the "natural order" that characters initially accept
     → Plant enforcement evidence characters can discover
     → Plan the moment of reclassification
     → Show: burden varies by power, enforcement visible at edges
   
   Purity drift:
     → Design the surface-level health indicators characters trust
     → Design the subtle decay markers analytical characters notice
     → Plan the crisis moment when surface metrics finally break
   
   Network contamination:
     → Design the healthy focal system characters rely on
     → Design the degraded neighbors exerting pressure
     → Show: compromises accumulating, workarounds normalizing
     → Plan the moment intrinsic integrity starts failing
   ```
...
<setting_design>
  ...
  <constraint_manifestations>
    <constraint source_id="C1" target_name="[New name]">
      <preserves>
        <epsilon>0.55 (routes to same type zone as source)</epsilon>
  ...
  <structural_physics_design>
    [If applicable: how False Mountain, purity drift, or contamination
     arcs are naturalized in the setting]
  </structural_physics_design>
...
☐ All constraints re-instantiated with routing preserved
    (same characters route to same types, not exact ε match)
☐ Indexical variance preserved (characters experience different types)
...
☐ Structural physics arcs naturalized (if applicable)
```
...
5. **(If structural physics arcs) Plan their dramatic beats:**
   ```xml
   <structural_physics_beats>
     <false_mountain_reveal>
       <setup>Characters accept C2 as natural/inevitable (acts 1-2)</setup>
       <evidence>Enforcement costs surface, burden asymmetry noticed</evidence>
       <reveal_moment>Character discovers the "natural law" has police</reveal_moment>
       <consequence>Reclassification opens new action possibilities</consequence>
     </false_mountain_reveal>
     
     <purity_drift_arc>
       <surface_health>System metrics look stable (acts 1-2)</surface_health>
       <subtle_decay>Theater increasing, coordination narrowing</subtle_decay>
       <cassandra_moment>Analytical character warns, gets dismissed</cassandra_moment>
       <crisis>Surface metrics break, type flip visible to all</crisis>
     </purity_drift_arc>
   </structural_physics_beats>
   ```

6. **Specify framework invisibility:**
   ```xml
   <invisibility_requirements>
     <banned_terms>
       index, power-scaling, Tangled Rope, extraction, coordination,
       effective extractiveness, chi, epsilon, constraint type, purity
     </banned_terms>
...
<story_blueprint>
  ...
  <structural_physics_beats>
    [If applicable: False Mountain, purity drift, contamination beats]
  </structural_physics_beats>
...
☐ Structural physics beats planned (if applicable)
```
...
4. **Final framework invisibility check:**
   ...
   - "purity": [count must be 0, in framework sense]
...
2. **Compare to Stage 1 specifications:**
     ...
     <constraint_preservation>
       <constraint source_id="C1">
         ...
         <epsilon_check>
           ...
           <output>0.58</output>
           <delta>0.03</delta>
           <routing_affected>no (both route to same types)</routing_affected>
           <pass>true</pass>
         </epsilon_check>
   ...
   **Key validation principle:** The primary test is **routing preservation** (same characters route to same constraint types), not **numeric fidelity** (exact ε match). If ε drifted from 0.55 to 0.62 but both characters still route to the same types with the same relative ordering, that's a pass. If ε drifted from 0.55 to 0.70 and the moderate character now routes to Snare instead of Tangled Rope, that's a fail — the topology broke.
...
3. **If FAIL, specify required fixes:**
   ...
   <required_fixes>
     ...
     <issue>Routing changed: moderate character routes to Snare in output
            but Tangled Rope in source (topology violation)</issue>
     <fix>Regenerate Stage 2 with tighter routing preservation</fix>
...
Metrics treated as truth → Reframe as routing (see metrics_as_routing.md)
Topology violated in output → Check routing preservation, not ε precision
```
...
- Detect structural signature overrides (FNL, CI_Rope, FCR)

**Mitigation (current):** The routing frame reduces the impact. LLM estimates are ±0.10 fuzzy at best, but if they route to the right types with correct relative ordering, the narrative output is sound. Structural physics concepts (§IX of logic_narrative.md) are available as narrative devices even without formal computation.

**Integration path (future):** If Prolog integration is added, the natural insertion points are:
- **Stage 0:** LLM estimates base properties → Prolog classifies → LLM uses authoritative types
- **Stage 5:** LLM re-extracts from output → Prolog validates classifications → formal topology check

This would give the pipeline both creative flexibility (LLM) and formal rigor (engine) without requiring the engine at every stage.
...
UKE_Narrative v1.3 (LLM-Optimized)  
Reference: `logic_narrative.md` for classification system and narrative applications  
