...
# Corpus composition (from output.txt)
echo -e "${BOLD}  CORPUS COMPOSITION (from test output)${NC}"
for ctype in mountain rope tangled_rope snare scaffold piton; do
    count=$(grep -c "Claimed Type: $ctype" "$OUTPUT_LOG" 2>/dev/null) || count=0
    printf "  %-16s %s\n" "$ctype:" "$count"
done
echo ""
